...
function App() {
  ...
  const getInitialMode = () => {
    ...
    return ['explore', 'season', 'career', 'compare', 'about', 'datasets'].includes(path) ? path : 'season';
  };

  const [datasetMode, setDatasetMode] = useState(getInitialMode());
  const DATASET_BASE_URL = `${window.location.origin}/torvik-datasets`;
  ...
  useEffect(() => {
    if (datasetMode === 'about' || datasetMode === 'datasets') return; // Don't fetch table data for info pages
  ...
  const activeFilters = useMemo(() => {
    if (datasetMode === 'compare' || datasetMode === 'about' || datasetMode === 'datasets') return [];
    const list = [];
    if (datasetMode === 'season' || datasetMode === 'explore') {
    ...
    <div className={`app-container ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${datasetMode === 'career' ? 'career-mode' : ''}`}>
      ...
      <div className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
        <div className="sidebar-header">
          ...
          <div className="dataset-toggle">
            ...
            <button className={datasetMode === 'datasets' ? 'active' : ''} onClick={() => setDatasetMode('datasets')}>Datasets</button>
        ...
        <div className="scroll-sidebar">
          ...
          ) : datasetMode === 'about' || datasetMode === 'datasets' ? (
      ...
      <div className="main-content">
        <div className="top-bar" style={{ paddingBottom: '10px' }}>
          <div className="top-bar-left">
            ...
            <div>
              <h1 style={{ marginBottom: '4px' }}>
                ...
                {datasetMode === 'datasets' && 'Torvik Extension Datasets'}
          ...
          {datasetMode !== 'about' && datasetMode !== 'datasets' && datasetMode !== 'explore' && (
        ...
        {datasetMode !== 'about' && datasetMode !== 'datasets' && activeFilters.length > 0 && (
        ...
        ) : datasetMode === 'datasets' ? (
          <div className="about-container">
            <div className="window about-window">
              <div className="about-header">
                <div className="about-title">Torvik Dataset Hosting</div>
                <div className="about-subtitle">Static JSON files for the Chrome extension are now hosted on this site.</div>
              </div>

              <div className="about-block">
                <div className="about-kicker">Live Endpoints</div>
                <ul className="about-list">
                  <li><strong>Manifest:</strong> <a href={`${DATASET_BASE_URL}/manifest.json`} target="_blank" rel="noreferrer">{`${DATASET_BASE_URL}/manifest.json`}</a></li>
                  <li><strong>2026 Dataset:</strong> <a href={`${DATASET_BASE_URL}/2026.json`} target="_blank" rel="noreferrer">{`${DATASET_BASE_URL}/2026.json`}</a></li>
                  <li><strong>Torvik Column Map:</strong> <a href={`${DATASET_BASE_URL}/torvik-column-map.json`} target="_blank" rel="noreferrer">{`${DATASET_BASE_URL}/torvik-column-map.json`}</a></li>
                </ul>
              </div>

              <div className="about-block">
                <div className="about-kicker">Extension Setup</div>
                <ul className="about-list">
                  <li>In the extension popup, set <strong>Season</strong> to <strong>2026</strong>.</li>
                  <li>Set URL to <code>{`${DATASET_BASE_URL}/2026.json`}</code>.</li>
                  <li>Set match key to <code>player</code>.</li>
                  <li>Inject columns like <code>team, cts_proj, cts_role, cts_notes</code>.</li>
                </ul>
              </div>

              <div className="about-block">
                <div className="about-kicker">Matching Rules (Based on Torvik HTML/JS)</div>
                <ul className="about-list">
                  <li>Torvik source array uses index <code>0</code> for player and <code>1</code> for team.</li>
                  <li>Best join key is normalized <code>player + team</code>, fallback to player-only.</li>
                  <li>If names differ, add aliases in your dataset so matching stays stable.</li>
                </ul>
              </div>
            </div>
          </div>
